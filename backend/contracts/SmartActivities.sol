...
contract SmartActivities is ERC20 {
  ...
  function getCompleteActivity(uint activityId) public view returns (CompleteActivity memory completeActivity) {
      ...
      completeActivity.inscribed = getInscribed(activityId, msg.sender);
      completeActivity.assisted = getAssisted(activityId, msg.sender);
      completeActivity.voted = getVoted(activityId, msg.sender);
  ...
  function inscribeToActivity(uint activityId) public {
      ...
      ActivityStatus storage activityStatus = activitiesStatus[activityId];
      activityStatus.inscribed[msg.sender] = true;
  }
